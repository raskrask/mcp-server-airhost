...
GCS_REMINDER_PREFIX = os.getenv("GCS_NOTIFIER_REMINDER_PREFIX", "airhost-notifier/reminded/")
REMINDER_HOUR = int(os.getenv("REMINDER_HOUR", "9"))  # この時刻(JST)の実行では当日チェックインの未完了予約も催促通知する
...
def _reminder_blob_name(reservation_id: str) -> str:
    return f"{GCS_REMINDER_PREFIX}{reservation_id}.json"


def is_reminded(bucket: storage.Bucket, reservation_id: str) -> bool:
    return bucket.blob(_reminder_blob_name(reservation_id)).exists()


def mark_reminded(bucket: storage.Bucket, reservation_id: str, info: dict) -> None:
    blob = bucket.blob(_reminder_blob_name(reservation_id))
    blob.upload_from_string(
        json.dumps(info, ensure_ascii=False, default=str),
        content_type="application/json",
    )
...
async def run() -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        today = now_jst.date()
        is_reminder_run = now_jst.hour == REMINDER_HOUR
        ...
        for res in reservations:
            ...
            if not reg.get("is_complete"):
                is_checkin_today = res["check_in"] == today.isoformat()
                if is_reminder_run and is_checkin_today and not is_reminded(bucket, rid):
